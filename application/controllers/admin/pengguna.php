...
class Pengguna extends CI_Controller
{
		...
		if (!empty($_FILES['filefoto']['name'])) {
			if ($this->upload->do_upload('filefoto')) {
				...
				if ($password <> $konfirm_password) {
					...
					$this->m_pengguna->simpan_pengguna($nama, $username, $password, $email, $nohp, $gambar);
		...
		if (!empty($_FILES['filefoto']['name'])) {
			if ($this->upload->do_upload('filefoto')) {
				...
				if (empty($password) && empty($konfirm_password)) {
					$this->m_pengguna->update_pengguna_tanpa_pass($kode, $nama, $username, $password, $email, $nohp, $gambar);
					...
					$this->m_pengguna->update_pengguna($kode, $nama, $username, $password, $email, $nohp, $gambar);
					echo $this->session->set_flashdata('msg', '<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert">&times;</button>Pengguna <b>' . $nama . '</b> Berhasil diupdate.</div>');
					redirect('admin/pengguna');
				}
			} else {
	...
	function keluhan()
	{
		...
		$data['datakeluhan'] = $this->m_keluhan->get_all_keluhan();
		$this->load->view('admin/v_datakeluhan', $data);
	}
}
